...
export default function TimelineView({ activities = [], date }) {
  ...
  const processed = useMemo(() => {
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      list.push({ ...current, isIdle: false });
      if (i < sorted.length - 1 && current.endTime) {
        const next = sorted[i + 1];
        const gapMs = new Date(next.startTime) - new Date(current.endTime);
        if (gapMs >= 1000) {
  ...
  return (
    <>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: 560 }}>
          <thead>
            <tr style={{ background: 'var(--color-bg-elevated)', borderBottom: '1px solid var(--color-border)' }}>
              <th style={thStyle({ width: 52 })}>Sr no</th>
          ...
          <tbody>
            {processed.map((a, index) => {
              const rowStyle = a.isIdle
                ? { borderBottom: '1px solid var(--color-border)', background: 'rgba(255,255,255,0.01)', opacity: 0.6 }
                : { borderBottom: '1px solid var(--color-border)' };
              const hasDescription = !a.isIdle && !!a.notes;
              return (
                <tr key={index} style={rowStyle}>
                  <td style={tdStyle({ color: 'var(--color-text-secondary)', fontFamily: 'JetBrains Mono' })}>
                    {index + 1}
                  </td>
                  <td
                    style={tdStyle({
                      ...
                      fontStyle: a.isIdle ? 'italic' : 'normal',
                    })}
                    onClick={() => hasDescription && setActiveActivity(a)}
                  >
                    <span style={{ marginRight: 7 }}>{a.emoji}</span>
                    {a.name}
                  </td>
                  <td style={tdStyle({ color: 'var(--color-text-secondary)', fontFamily: 'JetBrains Mono', fontSize: '0.82rem' })}>
                  ...
                  <td style={tdStyle({ color: 'var(--color-text-secondary)', fontFamily: 'JetBrains Mono', fontSize: '0.82rem' })}>
                    {a.endTime
                      ...
                      : (
                        <span style={{ color: 'var(--color-primary)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
                          <span className="animate-pulse">●</span> Ongoing
                        </span>
                      )}
                  </td>
                  <td style={tdStyle({ textAlign: 'right', fontFamily: 'JetBrains Mono', fontWeight: 600, color: 'var(--color-text-muted)', fontSize: '0.85rem' })}>
                    {a.isIdle ? formatDuration(a.duration) : '—'}
                  </td>
                  <td style={tdStyle({ textAlign: 'right', fontFamily: 'JetBrains Mono', fontWeight: 700, fontSize: '0.9rem', color: a.isIdle ? 'var(--color-text-muted)' : 'var(--color-primary-light)' })}>
                    {a.isIdle ? '—' : (a.duration != null ? formatDuration(a.duration) : (a.endTime ? '0s' : '-'))}
                  </td>
                </tr>
              );
            })}
      ...
      <div style={{ display: 'flex', gap: 20, marginTop: 10, fontSize: '0.75rem', color: 'var(--color-text-muted)', flexWrap: 'wrap' }}>
        <span><span style={{ color: 'var(--color-primary-light)', fontWeight: 700 }}>■</span> Total = productive time (paused time excluded)</span>
        <span><span style={{ opacity: 0.5 }}>■</span> Idle = gap between sessions</span>
      </div>
          ...
          <div className="status-picker-modal" style={{ maxWidth: 450, padding: 28, position: 'relative' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: 'var(--color-bg-elevated)', padding: 16, borderRadius: 'var(--radius-md)', fontSize: '0.9rem', color: 'var(--color-text-secondary)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div><strong>Productive Time:</strong> {activeActivity.duration ? formatDuration(activeActivity.duration) : '-'}</div>
                {(activeActivity.totalPausedDuration > 0) && (
                  <div><strong>Paused Time:</strong> {formatDuration(activeActivity.totalPausedDuration)}</div>
                )}
              </div>
